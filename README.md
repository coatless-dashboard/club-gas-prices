...
| Station | One station's price history per grade, its last thirty price changes, directions, its warehouse page where the source publishes one, and the five nearest active stations. Deep-linkable as `?station=<station_key>#station`. |
...
|---|---|---|
| `site-meta.json` | `data/meta.json` | The capture id, per-country status, the grade table, the notice, the basemap and the release links |
...
| `site-summary-daily.parquet` | `data/summary_daily.parquet` | Daily medians and quartiles by country and by region |
...
| `.github/workflows/test.yml` | Lint, `pytest`, and a full render and smoke test against the sample data on every pull request. |
...
MIT. See [LICENSE](LICENSE). The prices are Costco's; see the notice above.
